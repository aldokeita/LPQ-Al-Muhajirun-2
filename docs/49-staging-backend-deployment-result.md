...
Backend LPQ Al-Muhajirun 2 berhasil dideploy ke Supabase staging.
...
- Ref: `csvj...glqe`
- Name: `LPQ Al-Muhajirun Staging`
- Status: `ACTIVE_HEALTHY`

Repository berhasil dilink ke project staging dengan:

```text
supabase link --project-ref csvjeetirzdgebeoglqe
```
...
- `supabase projects list` tidak menampilkan project `csvj...glqe`;
- project yang muncul pada CLI bukan `LPQ Al-Muhajirun Staging`;
...
Alasan: akun/profile Supabase CLI yang aktif saat percobaan E2E tidak memiliki akses yang cocok ke project staging `csvj...glqe`. Melanjutkan bootstrap atau test terhadap remote pada kondisi ini berisiko menyasar project yang salah.

## Percobaan Validasi E2E Staging Kedua

Setelah login CLI diperbaiki, gate awal berhasil:

- `supabase projects list` menampilkan `LPQ Al-Muhajirun Staging` dengan ref `csvj...glqe`;
...
- hanya menerima target `https://csvjeetirzdgebeoglqe.supabase.co`;
...
Langkah aman berikutnya adalah memastikan Supabase CLI kembali login ke akun/organization yang menampilkan project `LPQ Al-Muhajirun Staging` dengan ref `csvj...glqe`.

Rekomendasi:

1. Jalankan ulang login Supabase CLI bila perlu.
2. Ulangi `supabase projects list`.
3. Lanjutkan hanya jika project `LPQ Al-Muhajirun Staging` dengan ref `csvj...glqe` tampil.
...
- `supabase projects list` pada terminal Codex kembali tidak menampilkan project `csvj...glqe`;
...
- project `LPQ Al-Muhajirun Staging` dengan ref `csvj...glqe` tetap tidak muncul pada terminal Codex;
...
- target dikunci ke `https://csvjeetirzdgebeoglqe.supabase.co`;
- menolak project ref selain `csvj...glqe`;
