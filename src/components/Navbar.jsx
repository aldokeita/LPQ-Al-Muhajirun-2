...
const NavLink = ({ to, children }) => (
  <Link to={to} className="text-foreground/80 hover:text-foreground transition-colors font-medium">
    {children}
  </Link>
);
...
const DropdownMenu = ({ title, children }) => {
  return (
    <div className="relative group">
      <button className="flex items-center space-x-1 text-foreground/80 hover:text-foreground transition-colors font-medium">
        <span>{title}</span>
        <ChevronDown className="w-4 h-4" />
      </button>
      <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-64 bg-card rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 border border-border">
        {children}
      </div>
    </div>
  );
...
const Navbar = () => {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-background/95 backdrop-blur-md shadow-sm transition-colors duration-300 border-b border-border/40">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <div className="flex items-center space-x-3">
             <Link to="/" className="flex items-center space-x-3">
              <img src={logoUrl} alt="Logo LPQ Al-Muhajirun" className="w-12 h-12 object-contain" />
              <span className="font-cinzel font-bold text-xl text-primary hidden sm:block">
                LPQ Al-Muhajirun
              </span>
            </Link>
          </div>
          ...
          <div className="hidden md:flex items-center space-x-2">
            {user ? (
              <>
                <Link to="/dashboard">
                  <Button variant="ghost" size="icon" className="text-primary hover:text-primary hover:bg-primary/10"><LayoutDashboard className="w-5 h-5"/></Button>
                </Link>
