  ...
  return (
    <div>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="santri-glass-modal max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-lg font-extrabold tracking-tight">{editingSantri ? 'Edit Santri Dewasa' : 'Tambah Santri Dewasa'}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="santri-glass-avatar-section sm:flex-row sm:items-center">
                <Avatar className="santri-glass-avatar w-24 h-24 cursor-pointer" onClick={() => formData.foto_url && setPreviewImage(formData.foto_url)}>
            ...
            <div className="santri-glass-card">
                ...
                <div className="santri-glass-card-body">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div><label className="santri-glass-label">Jenis Kelamin</label><Select value={formData.jenis_kelamin} onValueChange={val => setFormData({ ...formData, jenis_kelamin: val })}><SelectTrigger className="santri-glass-select bg-transparent border-transparent"><SelectValue /></SelectTrigger><SelectContent className="santri-glass-select-content bg-transparent border-transparent shadow-none rounded-none"><SelectItem value="Laki-laki">Laki-laki</SelectItem><SelectItem value="Perempuan">Perempuan</SelectItem></SelectContent></Select></div>
            ...
            <div className="santri-glass-card">
                ...
                <div className="santri-glass-card-body">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div><label className="santri-glass-label">Jilid</label><Select value={formData.jilid} onValueChange={val => setFormData({ ...formData, jilid: val })}><SelectTrigger className="santri-glass-select bg-transparent border-transparent"><SelectValue /></SelectTrigger><SelectContent className="santri-glass-select-content bg-transparent border-transparent shadow-none rounded-none">{jilidOptions.map(j => <SelectItem key={j} value={j}>{j}</SelectItem>)}</SelectContent></Select></div>
                        <div><label className="santri-glass-label">Sesi Mengaji</label><Select value={formData.sesi_mengaji} onValueChange={val => setFormData({ ...formData, sesi_mengaji: val })}><SelectTrigger className="santri-glass-select bg-transparent border-transparent"><SelectValue /></SelectTrigger><SelectContent className="santri-glass-select-content bg-transparent border-transparent shadow-none rounded-none">{sessionOptions.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent></Select></div>
                        <div><label className="santri-glass-label">Kelas Dewasa</label><Select value={formData.id_kelas || 'none'} onValueChange={val => setFormData({ ...formData, id_kelas: val === 'none' ? null : val })}><SelectTrigger className="santri-glass-select bg-transparent border-transparent"><SelectValue placeholder="Pilih Kelas" /></SelectTrigger><SelectContent className="santri-glass-select-content bg-transparent border-transparent shadow-none rounded-none"><SelectItem value="none">Belum Masuk Kelas</SelectItem>{classesList.map(c => <SelectItem key={c.id} value={c.id}>{c.nama_kelas}</SelectItem>)}</SelectContent></Select></div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                {editingSantri ? (
                    <button type="button" className="santri-glass-btn santri-glass-btn--migration" onClick={handleMigration}>
                ...
                <div className="flex gap-2">
                    ...
                    <button type="submit" className="santri-glass-btn santri-glass-btn--primary">
                        {editingSantri ? 'Simpan Perubahan' : 'Tambah Santri Dewasa'}
                    </button>
                </div>
            </div>
          </form>
