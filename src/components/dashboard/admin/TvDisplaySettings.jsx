...
const TvDisplaySettings = () => {
    ...
    const [config, setConfig] = useState({
        ...
        sessionQuotas: { pagi: 60, siang: 80, sore: 80 },
        registration: { startDate: '', endDate: '', contactWa: '628123456789', websiteUrl: 'lpqalmuhajirun.id' },
        leaderboard: {},
    ...
    const fetchSettings = async () => {
        setIsLoading(true);
        try {
            const { data: tvConfig } = await fetchWebsiteContentValue('tv_config');
            if (tvConfig) {
                setConfig(prev => ({ ...prev, ...tvConfig, sessionSettings: { ...prev.sessionSettings, ...tvConfig.sessionSettings }, sessionQuotas: { ...prev.sessionQuotas, ...tvConfig.sessionQuotas }, registration: { ...prev.registration, ...tvConfig.registration }, leaderboard: { ...prev.leaderboard, ...tvConfig.leaderboard }, durations: { ...prev.durations, ...tvConfig.durations }, showAdults: tvConfig.showAdults !== undefined ? tvConfig.showAdults : true }));
            }
    ...
    const saveConfig = async () => {
        setIsLoading(true);
        const { error } = await saveWebsiteContentValue('tv_config', config);
                    ...
                    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                        ...
                        <TabsContent value="quotas" className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 border-b pb-6">
                                ...
                                <div className="space-y-2"><Label className="flex items-center gap-2"><Globe className="w-4 h-4"/> Website URL</Label><Input placeholder="lpqalmuhajirun.id" value={config.registration?.websiteUrl || ''} onChange={(e) => setConfig(prev => ({ ...prev, registration: { ...prev.registration, websiteUrl: e.target.value } }))} /></div>
                            </div>
