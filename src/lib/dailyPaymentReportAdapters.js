...
export const printDailyPaymentHtml = ({ payments, summary, periodLabel, methodLabel }) => {
  ...
  printWindow.document.write(`<!doctype html>
    <html lang="id"><head><meta charset="utf-8"><title>Rekap SPP per Hari</title>
    <style>
    ...
    <h1>Rekap SPP per Hari</h1><p>LPQ Al-Muhajirun</p><p>${escapeHtml(periodLabel)} · ${escapeHtml(methodLabel)}</p>
    ...
    <table><thead><tr><th>No</th><th>Waktu</th><th>Santri</th><th>Kelas</th><th>Periode</th><th>Metode</th><th>Nominal</th><th>ID Transaksi</th></tr></thead><tbody>${tableRows}</tbody></table>
...
export const downloadDailyPaymentPdf = ({ payments, summary, periodLabel, methodLabel }) => {
  ...
  doc.setFontSize(16);
  doc.text('Rekap SPP per Hari', 14, 15);
  doc.setFontSize(10);
  ...
  doc.autoTable({
    startY: 34,
    head: [['No', 'Waktu', 'Santri', 'Kelas', 'Periode', 'Metode', 'Nominal', 'ID Transaksi']],
    body: buildDailyPaymentReportRows(payments),
    styles: { fontSize: 8, cellPadding: 2.2 },
    headStyles: { fillColor: [15, 118, 110] },
    columnStyles: { 0: { cellWidth: 10 }, 1: { cellWidth: 18 }, 6: { halign: 'right' } },
  });
  doc.save(`rekap-spp-harian-${periodLabel.replace(/\s+/g, '-').toLowerCase()}.pdf`);
};
