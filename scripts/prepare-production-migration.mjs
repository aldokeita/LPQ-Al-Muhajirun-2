...
const guru = guruSource.map((row) => {
  ...
  const technicalEmail = validEmail(row.email) ? normLower(row.email) : `guru+${row.id}@auth.lpqalmuhajirun.local`;
...
const santri = mergedSantri.map((row) => {
  ...
  const internalEmail = `santri+${id}@auth.lpqalmuhajirun.local`;
