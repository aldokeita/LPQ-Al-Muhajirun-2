...
$uri = [Uri]$SupabaseUrl
if ($uri.Scheme -ne "http" -or $uri.Host -notin @("127.0.0.1", "localhost", "::1") -or $uri.Port -ne 54321) {
...
select 'legacy_content_url_references|' || count(*) from public.website_content where content::text like '%wqnyoesvwnqfjqsbzmsi%';
