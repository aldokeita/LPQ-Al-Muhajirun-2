...
function Assert-LocalUrl {
  param([string]$Url)
  $uri = [Uri]$Url
  if ($uri.Scheme -ne "http" -or $uri.Port -ne 54321 -or ($uri.Host -notin @("127.0.0.1", "localhost", "::1"))) {
...
$adminLogin = SignInPassword -Email "admin-demo@example.invalid" -Password "LocalOnly-Demo-Admin-001!"
$guruALogin = SignInPassword -Email "guru-a-demo@example.invalid" -Password "LocalOnly-Demo-Guru-A-001!"
$guruBLogin = SignInPassword -Email "guru-b-demo@example.invalid" -Password "LocalOnly-Demo-Guru-B-001!"
if (-not $guruBLogin.ok) {
  $guruBLogin = SignInPassword -Email "guru-b-demo@example.invalid" -Password "LocalOnly-Demo-Guru-B-Reset!"
}
...
$santriLogin = CallFunction -Name "signin-with-nomor-induk" -Body @{ nomor_induk_qiroati = "DUMMYQA001"; password = "LocalOnly-Demo-Santri-A1!" }
...
$ownAvatar = CallFunction -Name "generate-signed-upload-url" -Token $santriToken -Body @{
  bucket = "avatars"
  path = "santri/10000000-0000-0000-0000-000000000101/profile.webp"
...
$otherAvatar = CallFunction -Name "generate-signed-upload-url" -Token $santriToken -Body @{
  bucket = "avatars"
  path = "santri/10000000-0000-0000-0000-000000000102/profile.webp"
  content_type = "image/webp"
  size = 1024
  purpose = "santri_avatar"
}
...
$duplicateSantri = CallFunction -Name "manage-user" -Token $adminToken -Body @{
  action = "create"
  role = "santri"
  initial_password = "LocalOnly-Demo-Duplicate-001!"
  profile = @{
    nomor_induk_qiroati = "DUMMYQA001"
...
$resetGuru = CallFunction -Name "reset-user-password" -Token $adminToken -Body @{
  ...
  new_password = "LocalOnly-Demo-Guru-B-Reset!"
  require_password_change = $true
}
Add-Result "function reset password admin allowed" ($resetGuru.ok) "status=$($resetGuru.status) error=$($resetGuru.error)"

$guruBResetLogin = SignInPassword -Email "guru-b-demo@example.invalid" -Password "LocalOnly-Demo-Guru-B-Reset!"
