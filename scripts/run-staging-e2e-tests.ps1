...
function Assert-StagingTarget {
  ...
  $uri = [Uri]$Url
  if ($Ref -ne "csvjeetirzdgebeoglqe" -or $uri.Scheme -ne "https" -or $uri.Host -ne "$Ref.supabase.co") {
    throw "Refusing non-staging target. Expected https://csvjeetirzdgebeoglqe.supabase.co."
  }
}
  ...
  Test-Case "reset password ditolak bagi non-admin" {
    try {
      Invoke-Function -Name "reset-user-password" -Headers $script:GuruHeaders -Body @{
        ...
        new_password = "DoNotUse-OnlyTesting-001!"
