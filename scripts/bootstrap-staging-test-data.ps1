param(
  [string]$SupabaseUrl = "https://csvjeetirzdgebeoglqe.supabase.co",
...
function Assert-StagingTarget {
  ...
  $uri = [Uri]$Url
  if ($uri.Scheme -ne "https" -or $uri.Host -ne "$Ref.supabase.co" -or $Ref -ne "csvjeetirzdgebeoglqe") {
    throw "Refusing to bootstrap staging data: target must be https://csvjeetirzdgebeoglqe.supabase.co."
  }
}
...
try {
  ...
  $classId = "20000000-0000-0000-0000-000000000101"
  ...
  $santriId = Ensure-ManagedUser -Role "santri" -AdminToken $adminToken -Profile @{
    ...
    nomor_induk_qiroati = "STGTEST001"
    kategori = "Anak"
  ...
  Upsert-ServiceRows -Table "website_content" -Rows @(
    @{
      ...
      key = "global_config"
      content = @{ site_name = "LPQ Al-Muhajirun Staging"; marker = "STAGING_DUMMY" }
      is_public = $true
      created_by = $admin.id
      updated_by = $admin.id
    },
  ...
  Write-Host "RFID marker: STAGING-RFID-001"
